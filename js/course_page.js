  ...
  .then((data) => {
    ...
    data.forEach((course) => {
      if (course.title === localStorage.courseTitle) {
        let tags = document.querySelector(".cour-head-section .cour-tags");
        ...
        // Headings
        let head = document.querySelector(".cour-head-section .cour-heading");
        ...
        // Enrolled
        let enrol = document.querySelector(".cour-details > div:nth-child(2)");
        ...
        details.innerHTML = course.overview.split("\n\n").join("</br></br>");
        // what you will learn
        ...
        // elements
        let ListItems = document.querySelectorAll(".cour-aside-list-element");
        ...
        accord.forEach((el, index) => {
          ...
          course[`step${index + 1}`].name.forEach((el, index) => {
            ...
            li.appendChild(i2);
            li.onclick = () => {
              location.href = course[`step${index + 1}`].link[index];
            };
            ul.appendChild(li);
          });
          let li = document.createElement("li");
          li.classList.add("accordion-list-elements", "quiz-element");
          li.setAttribute("data-link", course[`step${index + 1}`].quiz);
          let spans = document.createElement("span");
          let i = document.createElement("i");
