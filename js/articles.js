        ...
        .then((articles) => {
            articles.forEach((element, index) => {
                if (element.title == CurrentArticle) {
                    ...
                    let CurrentArt =
                        document.querySelector(".rootHead section");
                    CurrentArt.innerHTML = element.name;
                    // Start in the Blog Area
                    let ArtAuth = document.querySelector(
                        ".articles .head .name"
                    );
                    ArtAuth.innerHTML = element.name;
                    let MainHeader =
                        document.querySelector(".articles .head h1");
                    MainHeader.innerHTML = element.title;
                    let MainDate = document.querySelector(
                    ...
                    let BookSayQ = document.querySelector(
...
fetch(`js/blogs.json`)
    ...
    .then((blogs) => {
        let mine = blogs.filter(
            (ele) =>
                ele.name ==
                document.querySelector(".rootHead section").innerHTML.trim()
        );
        ...
        mine.forEach((ele, index) => {
            ...
            document
                .querySelector(".related-posts .content-post")
                .appendChild(blog);
            document
...
function goToArt(blg) {
    blg.addEventListener("click", () => {
        sessionStorage.Article =
            blg.childNodes[1].childNodes[1].innerHTML.trim();
